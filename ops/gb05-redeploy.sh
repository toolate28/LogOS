...
DIGEST=88b870e3011605d36d6d23bdd56c8b254e4bb1606168e700299a3e4c19965d6b
echo "DIGEST=${DIGEST}"

# Rewrite image pins with pure sed (no quote pollution)
sed -i -E "s|image: \"sha256:[a-f0-9']+\"|image: \"sha256:${DIGEST}\"|" compose.yaml
sed -i -E "s|image: reson8-waist@sha256:[a-f0-9']+|image: reson8-waist@sha256:${DIGEST}|" k8s/base/deployment.yaml
sed -i -E "s|digest: sha256:[a-f0-9']+|digest: sha256:${DIGEST}|" k8s/base/kustomization.yaml

echo "--- pins ---"
grep -nE 'image:|digest:' compose.yaml k8s/base/deployment.yaml k8s/base/kustomization.yaml
...
surface = {
    ...
    "notes": "Bookshelf not mounted; no Helm; no keys; image by digest; GET / is 404 by design; fakeRootCommands materializes /etc/passwd for containerd",
}
